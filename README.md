...
### 第二步：新增真实数据盲测，数据已入库、尚未运行

新增数据位于 `focimeter_system/data/real/multispot_lens_pairs/real_lens_repeat_set_002/`。已确认的镜片 1～3 各包含一张无镜片参考图、四张镜片保持不动的重复测量图、包装照片和商用焦度计 S/C/A 截图，共 12 个待测输入包。对应关系不确定的第 4 组未纳入可用数据集。详细配对、SHA-256、采集条件和使用边界见该目录的 `README.md` 与 `manifest.json`。

1. 使用已入库的 12 个输入包，先保持现有代码和参数不变直接运行。
2. 如果全部通过，确认当前实现具有初步泛化能力，随后启动 M4。
3. 如果失败，先做只读诊断，根据输入、日志和失败阶段判断属于采集、M1、M2 还是 M3。
4. 负责人再把修复任务交给实际责任板块；其他板块不得代改代码。
5. 修复后重新测试现有数据和新增数据，禁止只针对某一张图片写特殊规则。
