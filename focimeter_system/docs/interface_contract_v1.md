...
```json
{
  ...
  "error": {
    ...
    "message": "Detected spot count is below the configured minimum.",
    "module": "m2_image_recognition",
    "recoverable": true,
    "details": {}
  }
...
光斑识别输出中 `role` 是旧 5 点 mock 的兼容字段。正式 LM700 / Hartmann 多光斑输出可不提供 `role`；如果提供，只能使用以下值：
...
- 同一 `spot_id` 的已知 `role` 必须一致，不能通过排序或位置相近直接重编号。
...
当前实现不是对两张图分别排序后编号：M2 先在参考/标定图中建立光斑身份，再将测量图光斑匹配回参考/标定图。在现有合成测试和算法支持范围内，匹配成功时可以保证相同 `spot_id` 对应同一光斑；但完全对称结构可能产生 90° 身份别名，且当前没有真实多光斑成对图片，因此不能声明真实场景下无条件保证。
