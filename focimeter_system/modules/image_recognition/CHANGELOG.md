...
- 大尺度补充候选在晶格分析前受 `multispot_max_count` 剩余预算约束，复杂背景不再进入无界候选分析。
- 晶格恢复的信号门禁改为比较同一原始通道下的均值/峰值对比度，不再直接比较不同顶帽核产生的残差幅度。
- 补充大尺度候选风暴回归，并明确 README 中跨图配对只属于五点兼容模式。
- 真实 JPEG CLI 回归会重新生成四份 M2→M3 联调 JSON，并在规范化 JSON 后核对 SHA-256，防止检测逻辑变化后继续使用失配的固定样例，同时不受平台换行格式影响。
- 更新真实 JPEG 联调结论：默认正式模式继续安全拒绝，负责人显式启用 M3 工程模式时可输出 `software_only` 估算值，但仍不代表计量验证通过。
