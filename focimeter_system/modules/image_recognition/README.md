...
当前阶段版本：`0.4.0`
当前任务：第二阶段主任务。基于 `develop@db8c815` 创建新的 `task/m2-*` 分支，修复真实数据首轮盲测中的通用候选生成/背景预筛选问题。禁止只提高候选上限，禁止按文件名、镜片号、坐标或 S/C/A 写特例；不得修改 M3。详细验收条件见 `docs/TASK_BOARD_PHASE2_20260813.md`。
...
focimeter_m2_core（静态库）
  ...
  ├─ 轮廓筛选与亮度加权质心
  ├─ 标定图角色分配
  ├─ 五点兼容模式：受约束仿射变换枚举与跨图一一配对
  ├─ 实验多光斑模式：只输出单图 detection_id，不执行跨图配对
  └─ 结果、错误、质量诊断、日志和中间图输出
```
