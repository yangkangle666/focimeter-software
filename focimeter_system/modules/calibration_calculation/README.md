...
- 同一 `task_id` 下按 M2 继承的 `spot_id` 配对，并用 `role` 校验身份一致性；数组检测顺序可以变化
...
- 硬件参数待确认：
  ...
  - camera.pixel_size_um 当前为 TODO_CONFIRM，像元尺寸、传感器型号、镜头倍率、成像比例、中继光学系统必须由硬件组提供，软件组不能自行猜测。
  - 光源暂写 green_led，标注为临时参数；硬件组提供具体波长（如 520nm、525nm）后替换。
  - 软件可从图片自动读取 image_width / image_height。
- 测量目标尚未最终确定：当前 measurement_range 参考宁波法里奥 FL-8600/FL-800 竞品指标（球镜 ±25D、柱镜 ±10D、轴位 0°~180°、棱镜 0~15△），仅用于代码范围校验，标注为 REFERENCE_ONLY。最终指标以甲方/课程/项目验收要求为准。
- 当前协议中，`spot_id` 的硬性语义是“同一条物理光线”，不是检测数组序号。M2 不是对标定图和测量图分别排序编号，而是先在标定图建立光斑身份，再将测量图匹配回标定图；只有唯一受约束几何配对成功后，才为测量光斑继承标定光斑的 `spot_id` 和 `role`。在现有合成测试和算法支持范围内，匹配成功时可以保证相同 `spot_id` 对应同一光斑；ID 集合缺失、冲突、角色不一致、反射变换或方向反转都会被拒绝。
- 当前没有真实五光斑成对图片，完全对称结构还可能出现 90° 身份别名，因此不能声明真实场景下无条件保证。如果 M2 检测到配对不确定，M3 的后备方案是拒绝计算并返回 `COORDINATE_SYSTEM_INVALID`，由上游重新采集、人工确认或等待硬件/协议补充；M3 不使用最近点、排序或临时重编号生成结果。
